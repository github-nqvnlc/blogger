...
const FeaturePost = ({
  ...
  return (
    <div className="l-section-content">
      <div className="inner">
        <h1
          className={`text-6xl ${unbounded.className} font-bold leading-[1.2] bg-linear-to-r from-orange-accent-dark to-orange-vibrant text-transparent bg-clip-text
                    ${odd ? "text-right" : ""}`}
        >
          {title}
        </h1>
        <div className="grid grid-cols-12 gap-12 my-12">
          <div className={`col-span-6 ${odd ? "order-2" : "order-1"}`}>
            <a href={`/blog/${posts?.[0].slug}`}>
              <div className="cursor-pointer group">
                <div className="relative aspect-video">
                  <div className="absolute inset-0 shrink-0 overflow-hidden rounded-4xl">
                    <img
                      src={
                        posts?.[0].thumb
                      ...
                      className="object-cover w-full h-full aspect-video rounded-4xl transition-all duration-500 group-hover:scale-105 group-hover:opacity-100 group-hover:grayscale-0"
                      onError={e => {
                        e.currentTarget.src = PlaceholderImage.src;
                      }}
                    />
                  </div>
                  <div className="absolute inset-0 bg-linear-to-t from-black/70 to-black/0 rounded-4xl"></div>
                  <div className="absolute bottom-0 left-0 right-0 p-6 space-y-2 text-white">
                    <div className="flex items-center gap-2">
                      <Newspaper className="size-4" /> <span className="text-sm">{title}</span>
                    </div>
                    <div className="flex items-center gap-2">
                ...
                <div className="my-5 space-y-4">
                  <div className="font-bold text-blue-muted text-3xl">{posts?.[0].title}</div>
                  <div className="font-normal line-clamp-2">{posts?.[0].excerpt}</div>
                </div>
          ...
          <div className={`col-span-6 ${odd ? "order-1" : "order-2"}`}>
            <div className="relative flex flex-col gap-10">
              {posts?.slice(1, 4).map((post, index) => (
                <a href={`/blog/${post.slug}`} key={post.name ?? index}>
                  <div className="flex gap-6">
                    ...
                    <div className="flex flex-1 flex-col space-y-2">
                      <span className="text-xl font-bold text-blue-muted leading-snug">
                        {post.title}
                      </span>
                      <p className="text-sm text-foreground-variant line-clamp-2">
