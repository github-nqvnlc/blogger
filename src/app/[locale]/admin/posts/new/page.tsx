...
function formatFrappeDatetime(value: Date): string {
...
export default function CreatePostPage() {
  ...
  async function handleCreatePost() {
    ...
    try {
      const createdPost = await createPost.createDoc({
      ...
      let finalThumbUrl = form.thumb.trim();
      ...
      if (coverSource === "upload" && coverFileMeta?.name) {
        const attached = await updateFile.updateDoc(coverFileMeta.name, {
          ...
          attached_to_name: createdPost.name,
        ...
        if (finalThumbUrl && finalThumbUrl !== createdPost.thumb) {
          await updatePost.updateDoc(createdPost.name, {
            thumb: finalThumbUrl,
          });
        }
      }
      ...
      toast.success(copy.toast.createSuccess);
